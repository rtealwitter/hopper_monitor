...
Last updated: 2026-09-01T18:00:19-07:00
Samples: 1279 queue snapshots total (336 in the last 7 days), 1205 GPU snapshots total (317 in the last 7 days)
...
- **47.1%** (last 7 days) vs **41.1%** (all time) of the cluster's 60 GPUs allocated, averaged across samples
- **66.3%** (last 7 days) vs **54.2%** (all time) average `nvidia-smi` utilization *when* a GPU is allocated to a job
- **53.6%** (last 7 days) vs **70.5%** (all time) average cgroup CPU utilization *when* a CPU is allocated to a job
...
<tr style='background-color:#d8efef'><td>witter-lab</td><td>user-554c620c</td><td align='right'>4733.0</td><td align='right'>11384.0</td><td align='right'>66%</td><td align='right'>64%</td></tr>
<tr style='background-color:#d8efef'><td>witter-lab</td><td>user-f5bf0d80</td><td align='right'>9.0</td><td align='right'>41.5</td><td align='right'>20%</td><td align='right'>40%</td></tr>
...
Charts below cover the trailing 7 days: **2026-08-25 18:00 to 2026-09-01 18:00** (PDT).
...
One point per user per snapshot (n=924), usage decayed on Slurm's ~7-day fairshare half-life.
...
1. **Weight GPU usage in fairshare.** `TRESBillingWeights`/`PriorityWeightTRES` are unset, so idle GPUs cost nothing in priority - the failure mode the scatter above flags (upper-left: low CPU usage, high GPU usage). Fix: `scontrol update partition=main TRESBillingWeights=CPU=1.0,GRES/gpu=<weight>` then `scontrol reconfigure`. Start `<weight>` near the CPUs-per-GPU ratio (128/4=32) and tune against this week's numbers (47.1% allocated, 66.3% utilized when allocated) - a policy call, so loop in whoever owns cluster allocation.
2. **Escalate on sustained low utilization** (see table and scatter above). Current top candidate (most idle GPU-hours over the last 7d): `user-554c620c` in `witter-lab` - 1591 idle of 4733.0 GPU-hours allocated (66% utilization). Needs a utilization threshold (e.g. <20% mean over 50+ GPU-hours) and a grace period, then either a soft nudge (Slack/email) or a hard QOS penalty (`sacctmgr modify qos ... set Priority-=<n>`). Neither exists yet - this is read-only signal, not enforced policy.
