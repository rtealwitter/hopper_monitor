...
Last updated: 2026-09-02T07:00:09-07:00
Samples: 1305 queue snapshots total (336 in the last 7 days), 1231 GPU snapshots total (323 in the last 7 days)
...
- **64.3%** (last 7 days) vs **72.6%** (all time) average cgroup CPU utilization *when* a CPU is allocated to a job
...
<tr style='background-color:#d8efef'><td>witter-lab</td><td>user-554c620c</td><td align='right'>4648.5</td><td align='right'>11476.5</td><td align='right'>65%</td><td align='right'>64%</td></tr>
...
Charts below cover the trailing 7 days: **2026-08-26 07:00 to 2026-09-02 07:00** (PDT).
...
2. **Escalate on sustained low utilization** (see table and scatter above). Current top candidate (most idle GPU-hours over the last 7d): `user-554c620c` in `witter-lab` - 1606 idle of 4648.5 GPU-hours allocated (65% utilization). Needs a utilization threshold (e.g. <20% mean over 50+ GPU-hours) and a grace period, then either a soft nudge (Slack/email) or a hard QOS penalty (`sacctmgr modify qos ... set Priority-=<n>`). Neither exists yet - this is read-only signal, not enforced policy.
