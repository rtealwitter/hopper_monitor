...
Last updated: 2026-09-02T15:00:08-07:00
Samples: 1321 queue snapshots total (336 in the last 7 days), 1243 GPU snapshots total (322 in the last 7 days)
...
- **45.7%** (last 7 days) vs **40.0%** (all time) of the cluster's 60 GPUs allocated, averaged across samples
- **64.9%** (last 7 days) vs **54.0%** (all time) average `nvidia-smi` utilization *when* a GPU is allocated to a job
- **69.0%** (last 7 days) vs **73.9%** (all time) average cgroup CPU utilization *when* a CPU is allocated to a job
...
Charts below cover the trailing 7 days: **2026-08-26 15:00 to 2026-09-02 15:00** (PDT).
...
One point per user per snapshot (n=1016), usage decayed on Slurm's ~7-day fairshare half-life.
