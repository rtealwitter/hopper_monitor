...
Last updated: 2026-09-03T15:30:07-07:00
Samples: 1370 queue snapshots total (336 in the last 7 days), 1255 GPU snapshots total (295 in the last 7 days)
...
- **37.3%** (last 7 days) vs **38.6%** (all time) of the cluster's 60 GPUs allocated, averaged across samples
...
<tr style='background-color:#d8efef'><td>witter-lab</td><td>user-554c620c</td><td align='right'>3729.0</td><td align='right'>11481.0</td><td align='right'>64%</td><td align='right'>64%</td></tr>
...
Charts below cover the trailing 7 days: **2026-08-27 15:30 to 2026-09-03 15:30** (PDT).
...
Attribution combines `nvidia-smi`'s process listing with Slurm's GPU-to-job binding record; the latter caught **912** readings the former missed.
...
One point per user per snapshot (n=1025), usage decayed on Slurm's ~7-day fairshare half-life.
...
2. **Escalate on sustained low utilization** (see table and scatter above). Current top candidate (most idle GPU-hours over the last 7d): `user-554c620c` in `witter-lab` - 1330 idle of 3729.0 GPU-hours allocated (64% utilization). Needs a utilization threshold (e.g. <20% mean over 50+ GPU-hours) and a grace period, then either a soft nudge (Slack/email) or a hard QOS penalty (`sacctmgr modify qos ... set Priority-=<n>`). Neither exists yet - this is read-only signal, not enforced policy.
