...
Last updated: 2026-08-26T12:30:11-07:00
Samples: 980 queue snapshots total (313 in the last 7 days), 919 GPU snapshots total (277 in the last 7 days)
...
- **21.8%** (last 7 days) vs **38.3%** (all time) of the cluster's 60 GPUs allocated, averaged across samples
- **44.8%** (last 7 days) vs **49.8%** (all time) average `nvidia-smi` utilization *when* a GPU is allocated to a job
- **62.3%** (last 7 days) vs **77.0%** (all time) average cgroup CPU utilization *when* a CPU is allocated to a job
...
<tr style='background-color:#d8efef'><td>witter-lab</td><td>user-554c620c</td><td align='right'>1854.5</td><td align='right'>6883.0</td><td align='right'>47%</td><td align='right'>63%</td></tr>
...
Charts below cover the trailing 7 days: **2026-08-19 12:30 to 2026-08-26 12:30** (PDT).
...
2. **Escalate on sustained low utilization** (see table and scatter above). Current top candidate (most idle GPU-hours over the last 7d): `user-554c620c` in `witter-lab` - 978 idle of 1854.5 GPU-hours allocated (47% utilization). Needs a utilization threshold (e.g. <20% mean over 50+ GPU-hours) and a grace period, then either a soft nudge (Slack/email) or a hard QOS penalty (`sacctmgr modify qos ... set Priority-=<n>`). Neither exists yet - this is read-only signal, not enforced policy.
