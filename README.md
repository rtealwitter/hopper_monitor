...
Last updated: 2026-08-21T06:30:11-07:00
Samples: 751 queue snapshots total (334 in the last 7 days), 725 GPU snapshots total (308 in the last 7 days)
...
- **27.6%** (last 7 days) vs **42.9%** (all time) of the cluster's 60 GPUs allocated, averaged across samples
- **43.9%** (last 7 days) vs **50.6%** (all time) average `nvidia-smi` utilization *when* a GPU is allocated to a job
- **78.1%** (last 7 days) vs **80.1%** (all time) average cgroup CPU utilization *when* a CPU is allocated to a job
...
<tr style='background-color:#d8efef'><td>witter-lab</td><td>user-554c620c</td><td align='right'>1730.5</td><td align='right'>5471.0</td><td align='right'>60%</td><td align='right'>67%</td></tr>
...
<tr style='background-color:#e3e1f1'><td>zhuang-lab</td><td>user-0db9ced0</td><td align='right'>466.5</td><td align='right'>3043.5</td><td align='right'>25%</td><td align='right'>32%</td></tr>
...
Charts below cover the trailing 7 days: **2026-08-14 06:30 to 2026-08-21 06:30** (PDT).
...
Attribution combines `nvidia-smi`'s process listing with Slurm's GPU-to-job binding record; the latter caught **3577** readings the former missed.
...
2. **Escalate on sustained low utilization** (see table and scatter above). Current top candidate (most idle GPU-hours over the last 7d): `user-554c620c` in `witter-lab` - 688 idle of 1730.5 GPU-hours allocated (60% utilization). Needs a utilization threshold (e.g. <20% mean over 50+ GPU-hours) and a grace period, then either a soft nudge (Slack/email) or a hard QOS penalty (`sacctmgr modify qos ... set Priority-=<n>`). Neither exists yet - this is read-only signal, not enforced policy.
