...
Last updated: 2026-09-21T22:00:15-07:00
Samples: 2243 queue snapshots total (332 in the last 7 days), 2034 GPU snapshots total (289 in the last 7 days)
...
- **26.8%** (last 7 days) vs **50.2%** (all time) average `nvidia-smi` utilization *when* a GPU is allocated to a job
...
<tr style='background-color:#ededec'><td>zhuang-lab</td><td>user-0db9ced0</td><td align='right'>2563.0</td><td align='right'>8591.5</td><td align='right'>21%</td><td align='right'>25%</td></tr>
...
<tr style='background-color:#e3e1f1'><td>nerenberg-lab</td><td>user-7eb22d7c</td><td align='right'>298.0</td><td align='right'>568.5</td><td align='right'>—</td><td align='right'>67%</td></tr>
...
<tr style='background-color:#ededec'><td>zhuang-lab</td><td>user-e67a8f7c</td><td align='right'>37.0</td><td align='right'>38.0</td><td align='right'>71%</td><td align='right'>70%</td></tr>
...
Charts below cover the trailing 7 days: **2026-09-14 22:00 to 2026-09-21 22:00** (PDT).
...
Attribution combines `nvidia-smi`'s process listing with Slurm's GPU-to-job binding record; the latter caught **806** readings the former missed.
...
One point per user per snapshot (n=1469), usage decayed on Slurm's ~7-day fairshare half-life.
...
1. **Weight GPU usage in fairshare.** `TRESBillingWeights`/`PriorityWeightTRES` are unset, so idle GPUs cost nothing in priority - the failure mode the scatter above flags (upper-left: low CPU usage, high GPU usage). Fix: `scontrol update partition=main TRESBillingWeights=CPU=1.0,GRES/gpu=<weight>` then `scontrol reconfigure`. Start `<weight>` near the CPUs-per-GPU ratio (128/4=32) and tune against this week's numbers (36.3% allocated, 26.8% utilized when allocated) - a policy call, so loop in whoever owns cluster allocation.
2. **Escalate on sustained low utilization** (see table and scatter above). Current top candidate (most idle GPU-hours over the last 7d): `user-0db9ced0` in `zhuang-lab` - 2027 idle of 2563.0 GPU-hours allocated (21% utilization). Needs a utilization threshold (e.g. <20% mean over 50+ GPU-hours) and a grace period, then either a soft nudge (Slack/email) or a hard QOS penalty (`sacctmgr modify qos ... set Priority-=<n>`). Neither exists yet - this is read-only signal, not enforced policy.
