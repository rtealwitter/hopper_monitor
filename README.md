...
Last updated: 2026-08-24T06:30:05-07:00
Samples: 883 queue snapshots total (320 in the last 7 days), 838 GPU snapshots total (279 in the last 7 days)
...
- **23.7%** (last 7 days) vs **40.1%** (all time) of the cluster's 60 GPUs allocated, averaged across samples
- **39.8%** (last 7 days) vs **49.2%** (all time) average `nvidia-smi` utilization *when* a GPU is allocated to a job
- **70.7%** (last 7 days) vs **78.5%** (all time) average cgroup CPU utilization *when* a CPU is allocated to a job
...
<tr style='background-color:#d8efef'><td>witter-lab</td><td>user-554c620c</td><td align='right'>1746.5</td><td align='right'>6246.0</td><td align='right'>48%</td><td align='right'>63%</td></tr>
<tr style='background-color:#d8efef'><td>witter-lab</td><td>user-d58f5a15</td><td align='right'>388.0</td><td align='right'>1023.5</td><td align='right'>5%</td><td align='right'>7%</td></tr>
...
Charts below cover the trailing 7 days: **2026-08-17 06:30 to 2026-08-24 06:30** (PDT).
...
Attribution combines `nvidia-smi`'s process listing with Slurm's GPU-to-job binding record; the latter caught **2502** readings the former missed.
...
One point per user per snapshot (n=880), usage decayed on Slurm's ~7-day fairshare half-life.
...
1. **Weight GPU usage in fairshare.** `TRESBillingWeights`/`PriorityWeightTRES` are unset, so idle GPUs cost nothing in priority - the failure mode the scatter above flags (upper-left: low CPU usage, high GPU usage). Fix: `scontrol update partition=main TRESBillingWeights=CPU=1.0,GRES/gpu=<weight>` then `scontrol reconfigure`. Start `<weight>` near the CPUs-per-GPU ratio (128/4=32) and tune against this week's numbers (23.7% allocated, 39.8% utilized when allocated) - a policy call, so loop in whoever owns cluster allocation.
2. **Escalate on sustained low utilization** (see table and scatter above). Current top candidate (most idle GPU-hours over the last 7d): `user-554c620c` in `witter-lab` - 914 idle of 1746.5 GPU-hours allocated (48% utilization). Needs a utilization threshold (e.g. <20% mean over 50+ GPU-hours) and a grace period, then either a soft nudge (Slack/email) or a hard QOS penalty (`sacctmgr modify qos ... set Priority-=<n>`). Neither exists yet - this is read-only signal, not enforced policy.
