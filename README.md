...
Last updated: 2026-08-26T18:00:06-07:00
Samples: 991 queue snapshots total (313 in the last 7 days), 922 GPU snapshots total (270 in the last 7 days)
...
- **21.0%** (last 7 days) vs **37.9%** (all time) of the cluster's 60 GPUs allocated, averaged across samples
- **45.0%** (last 7 days) vs **49.8%** (all time) average `nvidia-smi` utilization *when* a GPU is allocated to a job
- **61.7%** (last 7 days) vs **76.9%** (all time) average cgroup CPU utilization *when* a CPU is allocated to a job
...
<tr style='background-color:#d8efef'><td>witter-lab</td><td>user-554c620c</td><td align='right'>1779.5</td><td align='right'>6893.5</td><td align='right'>48%</td><td align='right'>63%</td></tr>
...
<tr style='background-color:#fbebf1'><td>nerenberg-lab</td><td>user-fedb5feb</td><td align='right'>19.0</td><td align='right'>67.5</td><td align='right'>45%</td><td align='right'>58%</td></tr>
...
Charts below cover the trailing 7 days: **2026-08-19 18:00 to 2026-08-26 18:00** (PDT).
...
Attribution combines `nvidia-smi`'s process listing with Slurm's GPU-to-job binding record; the latter caught **1568** readings the former missed.
...
1. **Weight GPU usage in fairshare.** `TRESBillingWeights`/`PriorityWeightTRES` are unset, so idle GPUs cost nothing in priority - the failure mode the scatter above flags (upper-left: low CPU usage, high GPU usage). Fix: `scontrol update partition=main TRESBillingWeights=CPU=1.0,GRES/gpu=<weight>` then `scontrol reconfigure`. Start `<weight>` near the CPUs-per-GPU ratio (128/4=32) and tune against this week's numbers (21.0% allocated, 45.0% utilized when allocated) - a policy call, so loop in whoever owns cluster allocation.
2. **Escalate on sustained low utilization** (see table and scatter above). Current top candidate (most idle GPU-hours over the last 7d): `user-554c620c` in `witter-lab` - 932 idle of 1779.5 GPU-hours allocated (48% utilization). Needs a utilization threshold (e.g. <20% mean over 50+ GPU-hours) and a grace period, then either a soft nudge (Slack/email) or a hard QOS penalty (`sacctmgr modify qos ... set Priority-=<n>`). Neither exists yet - this is read-only signal, not enforced policy.
