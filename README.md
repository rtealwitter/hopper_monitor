...
Last updated: 2026-09-02T06:30:09-07:00
Samples: 1304 queue snapshots total (336 in the last 7 days), 1230 GPU snapshots total (323 in the last 7 days)
...
- **46.3%** (last 7 days) vs **40.5%** (all time) of the cluster's 60 GPUs allocated, averaged across samples
- **65.3%** (last 7 days) vs **54.0%** (all time) average `nvidia-smi` utilization *when* a GPU is allocated to a job
- **64.0%** (last 7 days) vs **72.6%** (all time) average cgroup CPU utilization *when* a CPU is allocated to a job
...
<tr style='background-color:#d8efef'><td>witter-lab</td><td>user-554c620c</td><td align='right'>4652.5</td><td align='right'>11475.5</td><td align='right'>66%</td><td align='right'>64%</td></tr>
...
Charts below cover the trailing 7 days: **2026-08-26 06:30 to 2026-09-02 06:30** (PDT).
...
2. **Escalate on sustained low utilization** (see table and scatter above). Current top candidate (most idle GPU-hours over the last 7d): `user-554c620c` in `witter-lab` - 1605 idle of 4652.5 GPU-hours allocated (66% utilization). Needs a utilization threshold (e.g. <20% mean over 50+ GPU-hours) and a grace period, then either a soft nudge (Slack/email) or a hard QOS penalty (`sacctmgr modify qos ... set Priority-=<n>`). Neither exists yet - this is read-only signal, not enforced policy.
