...
Last updated: 2026-08-25T13:30:26-07:00
Samples: 934 queue snapshots total (309 in the last 7 days), 880 GPU snapshots total (272 in the last 7 days)
...
- **44.5%** (last 7 days) vs **49.4%** (all time) average `nvidia-smi` utilization *when* a GPU is allocated to a job
- **67.4%** (last 7 days) vs **78.0%** (all time) average cgroup CPU utilization *when* a CPU is allocated to a job
...
<tr style='background-color:#d8efef'><td>witter-lab</td><td>user-554c620c</td><td align='right'>1865.0</td><td align='right'>6595.5</td><td align='right'>47%</td><td align='right'>63%</td></tr>
...
Charts below cover the trailing 7 days: **2026-08-18 13:30 to 2026-08-25 13:30** (PDT).
...
One point per user per snapshot (n=736), usage decayed on Slurm's ~7-day fairshare half-life.
...
2. **Escalate on sustained low utilization** (see table and scatter above). Current top candidate (most idle GPU-hours over the last 7d): `user-554c620c` in `witter-lab` - 984 idle of 1865.0 GPU-hours allocated (47% utilization). Needs a utilization threshold (e.g. <20% mean over 50+ GPU-hours) and a grace period, then either a soft nudge (Slack/email) or a hard QOS penalty (`sacctmgr modify qos ... set Priority-=<n>`). Neither exists yet - this is read-only signal, not enforced policy.
