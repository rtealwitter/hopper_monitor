...
Last updated: 2026-08-09T23:00:19-07:00
Samples: 210 queue snapshots total (210 in the last 7 days), 210 GPU snapshots total (210 in the last 7 days)
...
- **47.9%** (last 7 days) vs **47.9%** (all time) of the cluster's 60 GPUs allocated, averaged across samples
- **51.0%** (last 7 days) vs **51.0%** (all time) average `nvidia-smi` utilization *when* a GPU is allocated to a job
- **81.1%** (last 7 days) vs **81.1%** (all time) average cgroup CPU utilization *when* a CPU is allocated to a job

## Most open times

Based on 5 days of history so far, `hopper.cluster` has historically been most open on **Saturdays** (29.6% of GPUs allocated on average) and around **22:00-23:00** (33.7% of GPUs allocated on average). Recomputed from all-time data on every cron tick, so it sharpens as more history accumulates.
...
<tr style='background-color:#e3e1f1'><td>zhuang-lab</td><td>user-0db9ced0</td><td align='right'>1321.5</td><td align='right'>1321.5</td><td align='right'>39%</td><td align='right'>39%</td></tr>
<tr style='background-color:#d8efef'><td>witter-lab</td><td>user-554c620c</td><td align='right'>1165.5</td><td align='right'>1165.5</td><td align='right'>72%</td><td align='right'>72%</td></tr>
...
Charts below cover the trailing 7 days: **2026-08-02 23:00 to 2026-08-09 23:00** (PDT).
...
Attribution combines `nvidia-smi`'s process listing with Slurm's GPU-to-job binding record; the latter caught **3974** readings the former missed.
...
One point per user per snapshot (n=948), usage decayed on Slurm's ~7-day fairshare half-life.
