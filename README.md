...
Last updated: 2026-09-01T00:30:12-07:00
Samples: 1244 queue snapshots total (332 in the last 7 days), 1170 GPU snapshots total (311 in the last 7 days)
...
- **47.3%** (last 7 days) vs **41.5%** (all time) of the cluster's 60 GPUs allocated, averaged across samples
- **67.4%** (last 7 days) vs **54.3%** (all time) average `nvidia-smi` utilization *when* a GPU is allocated to a job
- **45.4%** (last 7 days) vs **69.7%** (all time) average cgroup CPU utilization *when* a CPU is allocated to a job

## Most open times

Based on 28 days of history so far, `hopper.cluster` has historically been most open on **Saturdays** (25.8% of GPUs allocated on average) and around **08:00-09:00** (34.2% of GPUs allocated on average). Recomputed from all-time data on every cron tick, so it sharpens as more history accumulates.
...
<tr style='background-color:#d8efef'><td>witter-lab</td><td>user-554c620c</td><td align='right'>4698.5</td><td align='right'>11107.0</td><td align='right'>67%</td><td align='right'>65%</td></tr>
...
Charts below cover the trailing 7 days: **2026-08-25 00:30 to 2026-09-01 00:30** (PDT).
...
Attribution combines `nvidia-smi`'s process listing with Slurm's GPU-to-job binding record; the latter caught **833** readings the former missed.
...
One point per user per snapshot (n=834), usage decayed on Slurm's ~7-day fairshare half-life.
