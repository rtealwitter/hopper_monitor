...
Last updated: 2026-09-02T11:00:11-07:00
Samples: 1313 queue snapshots total (336 in the last 7 days), 1237 GPU snapshots total (321 in the last 7 days)
...
- **45.9%** (last 7 days) vs **40.3%** (all time) of the cluster's 60 GPUs allocated, averaged across samples
- **65.0%** (last 7 days) vs **54.0%** (all time) average `nvidia-smi` utilization *when* a GPU is allocated to a job
- **66.8%** (last 7 days) vs **73.3%** (all time) average cgroup CPU utilization *when* a CPU is allocated to a job
...
<tr style='background-color:#d8efef'><td>witter-lab</td><td>user-554c620c</td><td align='right'>4609.5</td><td align='right'>11477.5</td><td align='right'>65%</td><td align='right'>64%</td></tr>
<tr style='background-color:#d8efef'><td>witter-lab</td><td>user-f5bf0d80</td><td align='right'>16.5</td><td align='right'>49.0</td><td align='right'>17%</td><td align='right'>35%</td></tr>
...
Charts below cover the trailing 7 days: **2026-08-26 11:00 to 2026-09-02 11:00** (PDT).
...
One point per user per snapshot (n=997), usage decayed on Slurm's ~7-day fairshare half-life.
...
2. **Escalate on sustained low utilization** (see table and scatter above). Current top candidate (most idle GPU-hours over the last 7d): `user-554c620c` in `witter-lab` - 1598 idle of 4609.5 GPU-hours allocated (65% utilization). Needs a utilization threshold (e.g. <20% mean over 50+ GPU-hours) and a grace period, then either a soft nudge (Slack/email) or a hard QOS penalty (`sacctmgr modify qos ... set Priority-=<n>`). Neither exists yet - this is read-only signal, not enforced policy.
