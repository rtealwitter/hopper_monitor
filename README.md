...
Last updated: 2026-08-25T04:30:09-07:00
Samples: 920 queue snapshots total (313 in the last 7 days), 867 GPU snapshots total (277 in the last 7 days)
...
- **23.5%** (last 7 days) vs **39.2%** (all time) of the cluster's 60 GPUs allocated, averaged across samples
- **40.2%** (last 7 days) vs **49.3%** (all time) average `nvidia-smi` utilization *when* a GPU is allocated to a job
- **67.8%** (last 7 days) vs **78.2%** (all time) average cgroup CPU utilization *when* a CPU is allocated to a job

## Most open times

Based on 21 days of history so far, `hopper.cluster` has historically been most open on **Saturdays** (26.9% of GPUs allocated on average) and around **08:00-09:00** (33.6% of GPUs allocated on average). Recomputed from all-time data on every cron tick, so it sharpens as more history accumulates.
...
<tr style='background-color:#d8efef'><td>witter-lab</td><td>user-554c620c</td><td align='right'>1742.5</td><td align='right'>6473.0</td><td align='right'>47%</td><td align='right'>63%</td></tr>
<tr style='background-color:#d8efef'><td>witter-lab</td><td>user-d58f5a15</td><td align='right'>324.0</td><td align='right'>1023.5</td><td align='right'>5%</td><td align='right'>7%</td></tr>
...
Charts below cover the trailing 7 days: **2026-08-18 04:30 to 2026-08-25 04:30** (PDT).
...
Attribution combines `nvidia-smi`'s process listing with Slurm's GPU-to-job binding record; the latter caught **2306** readings the former missed.
...
One point per user per snapshot (n=792), usage decayed on Slurm's ~7-day fairshare half-life.
...
1. **Weight GPU usage in fairshare.** `TRESBillingWeights`/`PriorityWeightTRES` are unset, so idle GPUs cost nothing in priority - the failure mode the scatter above flags (upper-left: low CPU usage, high GPU usage). Fix: `scontrol update partition=main TRESBillingWeights=CPU=1.0,GRES/gpu=<weight>` then `scontrol reconfigure`. Start `<weight>` near the CPUs-per-GPU ratio (128/4=32) and tune against this week's numbers (23.5% allocated, 40.2% utilized when allocated) - a policy call, so loop in whoever owns cluster allocation.
2. **Escalate on sustained low utilization** (see table and scatter above). Current top candidate (most idle GPU-hours over the last 7d): `user-554c620c` in `witter-lab` - 927 idle of 1742.5 GPU-hours allocated (47% utilization). Needs a utilization threshold (e.g. <20% mean over 50+ GPU-hours) and a grace period, then either a soft nudge (Slack/email) or a hard QOS penalty (`sacctmgr modify qos ... set Priority-=<n>`). Neither exists yet - this is read-only signal, not enforced policy.
