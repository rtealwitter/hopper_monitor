...
Last updated: 2026-08-29T05:30:09-07:00
Samples: 1110 queue snapshots total (313 in the last 7 days), 1036 GPU snapshots total (274 in the last 7 days)
...
- **34.0%** (last 7 days) vs **39.5%** (all time) of the cluster's 60 GPUs allocated, averaged across samples
...
Based on 25 days of history so far, `hopper.cluster` has historically been most open on **Saturdays** (25.6% of GPUs allocated on average) and around **08:00-09:00** (33.7% of GPUs allocated on average). Recomputed from all-time data on every cron tick, so it sharpens as more history accumulates.
...
<tr style='background-color:#d8efef'><td>witter-lab</td><td>user-554c620c</td><td align='right'>3101.0</td><td align='right'>8771.0</td><td align='right'>58%</td><td align='right'>63%</td></tr>
...
Charts below cover the trailing 7 days: **2026-08-22 05:30 to 2026-08-29 05:30** (PDT).
...
Attribution combines `nvidia-smi`'s process listing with Slurm's GPU-to-job binding record; the latter caught **1279** readings the former missed.
...
2. **Escalate on sustained low utilization** (see table and scatter above). Current top candidate (most idle GPU-hours over the last 7d): `user-554c620c` in `witter-lab` - 1316 idle of 3101.0 GPU-hours allocated (58% utilization). Needs a utilization threshold (e.g. <20% mean over 50+ GPU-hours) and a grace period, then either a soft nudge (Slack/email) or a hard QOS penalty (`sacctmgr modify qos ... set Priority-=<n>`). Neither exists yet - this is read-only signal, not enforced policy.
