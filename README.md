...
Last updated: 2026-08-22T04:00:10-07:00
Samples: 794 queue snapshots total (334 in the last 7 days), 762 GPU snapshots total (302 in the last 7 days)
...
- **26.6%** (last 7 days) vs **41.8%** (all time) of the cluster's 60 GPUs allocated, averaged across samples
- **42.6%** (last 7 days) vs **50.0%** (all time) average `nvidia-smi` utilization *when* a GPU is allocated to a job
- **75.3%** (last 7 days) vs **79.6%** (all time) average cgroup CPU utilization *when* a CPU is allocated to a job

## Most open times

Based on 18 days of history so far, `hopper.cluster` has historically been most open on **Saturdays** (28.3% of GPUs allocated on average) and around **01:00-02:00** (35.6% of GPUs allocated on average). Recomputed from all-time data on every cron tick, so it sharpens as more history accumulates.
...
<tr style='background-color:#e3e1f1'><td>zhuang-lab</td><td>user-0db9ced0</td><td align='right'>272.0</td><td align='right'>3122.0</td><td align='right'>17%</td><td align='right'>31%</td></tr>
...
Charts below cover the trailing 7 days: **2026-08-15 04:00 to 2026-08-22 04:00** (PDT).
...
Attribution combines `nvidia-smi`'s process listing with Slurm's GPU-to-job binding record; the latter caught **3399** readings the former missed.
