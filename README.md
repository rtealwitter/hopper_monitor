...
Last updated: 2026-09-02T09:30:09-07:00
Samples: 1310 queue snapshots total (336 in the last 7 days), 1234 GPU snapshots total (321 in the last 7 days)
...
- **46.0%** (last 7 days) vs **40.4%** (all time) of the cluster's 60 GPUs allocated, averaged across samples
- **65.1%** (last 7 days) vs **54.0%** (all time) average `nvidia-smi` utilization *when* a GPU is allocated to a job
- **65.9%** (last 7 days) vs **73.0%** (all time) average cgroup CPU utilization *when* a CPU is allocated to a job
...
<tr style='background-color:#d8efef'><td>witter-lab</td><td>user-554c620c</td><td align='right'>4624.5</td><td align='right'>11477.5</td><td align='right'>65%</td><td align='right'>64%</td></tr>
<tr style='background-color:#d8efef'><td>witter-lab</td><td>user-f5bf0d80</td><td align='right'>13.5</td><td align='right'>46.0</td><td align='right'>19%</td><td align='right'>37%</td></tr>
...
Charts below cover the trailing 7 days: **2026-08-26 09:30 to 2026-09-02 09:30** (PDT).
...
One point per user per snapshot (n=991), usage decayed on Slurm's ~7-day fairshare half-life.
...
2. **Escalate on sustained low utilization** (see table and scatter above). Current top candidate (most idle GPU-hours over the last 7d): `user-554c620c` in `witter-lab` - 1602 idle of 4624.5 GPU-hours allocated (65% utilization). Needs a utilization threshold (e.g. <20% mean over 50+ GPU-hours) and a grace period, then either a soft nudge (Slack/email) or a hard QOS penalty (`sacctmgr modify qos ... set Priority-=<n>`). Neither exists yet - this is read-only signal, not enforced policy.
