...
Last updated: 2026-08-25T14:30:19-07:00
Samples: 936 queue snapshots total (311 in the last 7 days), 882 GPU snapshots total (274 in the last 7 days)
...
- **22.8%** (last 7 days) vs **39.2%** (all time) of the cluster's 60 GPUs allocated, averaged across samples
- **44.8%** (last 7 days) vs **49.4%** (all time) average `nvidia-smi` utilization *when* a GPU is allocated to a job
- **67.1%** (last 7 days) vs **77.9%** (all time) average cgroup CPU utilization *when* a CPU is allocated to a job
...
<tr style='background-color:#d8efef'><td>witter-lab</td><td>user-554c620c</td><td align='right'>1901.0</td><td align='right'>6631.5</td><td align='right'>48%</td><td align='right'>63%</td></tr>
...
Charts below cover the trailing 7 days: **2026-08-18 14:30 to 2026-08-25 14:30** (PDT).
...
2. **Escalate on sustained low utilization** (see table and scatter above). Current top candidate (most idle GPU-hours over the last 7d): `user-554c620c` in `witter-lab` - 997 idle of 1901.0 GPU-hours allocated (48% utilization). Needs a utilization threshold (e.g. <20% mean over 50+ GPU-hours) and a grace period, then either a soft nudge (Slack/email) or a hard QOS penalty (`sacctmgr modify qos ... set Priority-=<n>`). Neither exists yet - this is read-only signal, not enforced policy.
