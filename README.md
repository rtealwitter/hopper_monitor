...
Last updated: 2026-08-15T10:55:01-07:00
Samples: 474 queue snapshots total (337 in the last 7 days), 473 GPU snapshots total (336 in the last 7 days)
...
- **55.0%** (last 7 days) vs **52.1%** (all time) of the cluster's 60 GPUs allocated, averaged across samples
- **54.3%** (last 7 days) vs **52.4%** (all time) average `nvidia-smi` utilization *when* a GPU is allocated to a job
- **79.2%** (last 7 days) vs **81.6%** (all time) average cgroup CPU utilization *when* a CPU is allocated to a job

## Most open times

Based on 11 days of history so far, `hopper.cluster` has historically been most open on **Saturdays** (28.9% of GPUs allocated on average) and around **01:00-02:00** (43.8% of GPUs allocated on average). Recomputed from all-time data on every cron tick, so it sharpens as more history accumulates.
...
<tr style='background-color:#d8efef'><td>witter-lab</td><td>user-554c620c</td><td align='right'>3400.7</td><td align='right'>3887.7</td><td align='right'>71%</td><td align='right'>70%</td></tr>
<tr style='background-color:#e3e1f1'><td>zhuang-lab</td><td>user-0db9ced0</td><td align='right'>2096.0</td><td align='right'>2930.5</td><td align='right'>27%</td><td align='right'>32%</td></tr>
...
Charts below cover the trailing 7 days: **2026-08-08 10:55 to 2026-08-15 10:55** (PDT).
...
Attribution combines `nvidia-smi`'s process listing with Slurm's GPU-to-job binding record; the latter caught **7265** readings the former missed.
...
1. **Weight GPU usage in fairshare.** `TRESBillingWeights`/`PriorityWeightTRES` are unset, so idle GPUs cost nothing in priority - the failure mode the scatter above flags (upper-left: low CPU usage, high GPU usage). Fix: `scontrol update partition=main TRESBillingWeights=CPU=1.0,GRES/gpu=<weight>` then `scontrol reconfigure`. Start `<weight>` near the CPUs-per-GPU ratio (128/4=32) and tune against this week's numbers (55.0% allocated, 54.3% utilized when allocated) - a policy call, so loop in whoever owns cluster allocation.
2. **Escalate on sustained low utilization** (see table and scatter above). Current top candidate: `user-0db9ced0` in `zhuang-lab` - 2096.0 GPU-hours at 27% utilization (~1528 idle). Needs a utilization threshold (e.g. <20% mean over 50+ GPU-hours) and a grace period, then either a soft nudge (Slack/email) or a hard QOS penalty (`sacctmgr modify qos ... set Priority-=<n>`). Neither exists yet - this is read-only signal, not enforced policy.
