...
Last updated: 2026-08-27T04:00:27-07:00
Samples: 1011 queue snapshots total (313 in the last 7 days), 937 GPU snapshots total (265 in the last 7 days)
...
- **23.7%** (last 7 days) vs **38.0%** (all time) of the cluster's 60 GPUs allocated, averaged across samples
- **47.5%** (last 7 days) vs **50.2%** (all time) average `nvidia-smi` utilization *when* a GPU is allocated to a job
- **61.4%** (last 7 days) vs **76.7%** (all time) average cgroup CPU utilization *when* a CPU is allocated to a job
...
<tr style='background-color:#d8efef'><td>witter-lab</td><td>user-554c620c</td><td align='right'>2039.5</td><td align='right'>7166.5</td><td align='right'>50%</td><td align='right'>64%</td></tr>
...
<tr style='background-color:#fbebf1'><td>nerenberg-lab</td><td>user-fedb5feb</td><td align='right'>9.0</td><td align='right'>67.5</td><td align='right'>45%</td><td align='right'>58%</td></tr>
...
Charts below cover the trailing 7 days: **2026-08-20 04:00 to 2026-08-27 04:00** (PDT).
...
Attribution combines `nvidia-smi`'s process listing with Slurm's GPU-to-job binding record; the latter caught **1535** readings the former missed.
...
One point per user per snapshot (n=628), usage decayed on Slurm's ~7-day fairshare half-life.
...
1. **Weight GPU usage in fairshare.** `TRESBillingWeights`/`PriorityWeightTRES` are unset, so idle GPUs cost nothing in priority - the failure mode the scatter above flags (upper-left: low CPU usage, high GPU usage). Fix: `scontrol update partition=main TRESBillingWeights=CPU=1.0,GRES/gpu=<weight>` then `scontrol reconfigure`. Start `<weight>` near the CPUs-per-GPU ratio (128/4=32) and tune against this week's numbers (23.7% allocated, 47.5% utilized when allocated) - a policy call, so loop in whoever owns cluster allocation.
2. **Escalate on sustained low utilization** (see table and scatter above). Current top candidate (most idle GPU-hours over the last 7d): `user-554c620c` in `witter-lab` - 1016 idle of 2039.5 GPU-hours allocated (50% utilization). Needs a utilization threshold (e.g. <20% mean over 50+ GPU-hours) and a grace period, then either a soft nudge (Slack/email) or a hard QOS penalty (`sacctmgr modify qos ... set Priority-=<n>`). Neither exists yet - this is read-only signal, not enforced policy.
