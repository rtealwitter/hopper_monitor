...
Last updated: 2026-08-23T15:00:05-07:00
Samples: 864 queue snapshots total (332 in the last 7 days), 828 GPU snapshots total (300 in the last 7 days)
...
<tr style='background-color:#d8efef'><td>witter-lab</td><td>user-554c620c</td><td align='right'>1875.5</td><td align='right'>6178.0</td><td align='right'>50%</td><td align='right'>63%</td></tr>
...
Charts below cover the trailing 7 days: **2026-08-16 15:00 to 2026-08-23 15:00** (PDT).
...
One point per user per snapshot (n=941), usage decayed on Slurm's ~7-day fairshare half-life.
...
2. **Escalate on sustained low utilization** (see table and scatter above). Current top candidate (most idle GPU-hours over the last 7d): `user-554c620c` in `witter-lab` - 939 idle of 1875.5 GPU-hours allocated (50% utilization). Needs a utilization threshold (e.g. <20% mean over 50+ GPU-hours) and a grace period, then either a soft nudge (Slack/email) or a hard QOS penalty (`sacctmgr modify qos ... set Priority-=<n>`). Neither exists yet - this is read-only signal, not enforced policy.
