...
Last updated: 2026-08-30T09:00:29-07:00
Samples: 1165 queue snapshots total (313 in the last 7 days), 1091 GPU snapshots total (275 in the last 7 days)
...
- **33.9%** (last 7 days) vs **38.8%** (all time) of the cluster's 60 GPUs allocated, averaged across samples
- **60.5%** (last 7 days) vs **51.7%** (all time) average `nvidia-smi` utilization *when* a GPU is allocated to a job
...
<tr style='background-color:#d8efef'><td>witter-lab</td><td>user-554c620c</td><td align='right'>3180.0</td><td align='right'>9206.0</td><td align='right'>61%</td><td align='right'>63%</td></tr>
...
Charts below cover the trailing 7 days: **2026-08-23 09:00 to 2026-08-30 09:00** (PDT).
...
One point per user per snapshot (n=659), usage decayed on Slurm's ~7-day fairshare half-life.
...
1. **Weight GPU usage in fairshare.** `TRESBillingWeights`/`PriorityWeightTRES` are unset, so idle GPUs cost nothing in priority - the failure mode the scatter above flags (upper-left: low CPU usage, high GPU usage). Fix: `scontrol update partition=main TRESBillingWeights=CPU=1.0,GRES/gpu=<weight>` then `scontrol reconfigure`. Start `<weight>` near the CPUs-per-GPU ratio (128/4=32) and tune against this week's numbers (33.9% allocated, 60.5% utilized when allocated) - a policy call, so loop in whoever owns cluster allocation.
2. **Escalate on sustained low utilization** (see table and scatter above). Current top candidate (most idle GPU-hours over the last 7d): `user-554c620c` in `witter-lab` - 1255 idle of 3180.0 GPU-hours allocated (61% utilization). Needs a utilization threshold (e.g. <20% mean over 50+ GPU-hours) and a grace period, then either a soft nudge (Slack/email) or a hard QOS penalty (`sacctmgr modify qos ... set Priority-=<n>`). Neither exists yet - this is read-only signal, not enforced policy.
