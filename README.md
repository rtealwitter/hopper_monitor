...
Last updated: 2026-09-09T04:30:13-07:00
Samples: 1636 queue snapshots total (336 in the last 7 days), 1470 GPU snapshots total (244 in the last 7 days)
...
- **19.9%** (last 7 days) vs **36.4%** (all time) of the cluster's 60 GPUs allocated, averaged across samples
- **41.5%** (last 7 days) vs **52.6%** (all time) average `nvidia-smi` utilization *when* a GPU is allocated to a job
...
<tr style='background-color:#ededec'><td>zhuang-lab</td><td>user-0db9ced0</td><td align='right'>1195.5</td><td align='right'>4400.5</td><td align='right'>20%</td><td align='right'>28%</td></tr>
<tr style='background-color:#d8efef'><td>witter-lab</td><td>user-554c620c</td><td align='right'>706.0</td><td align='right'>12177.5</td><td align='right'>77%</td><td align='right'>65%</td></tr>
...
Charts below cover the trailing 7 days: **2026-09-02 04:30 to 2026-09-09 04:30** (PDT).
...
Attribution combines `nvidia-smi`'s process listing with Slurm's GPU-to-job binding record; the latter caught **612** readings the former missed.
...
One point per user per snapshot (n=1125), usage decayed on Slurm's ~7-day fairshare half-life.
...
1. **Weight GPU usage in fairshare.** `TRESBillingWeights`/`PriorityWeightTRES` are unset, so idle GPUs cost nothing in priority - the failure mode the scatter above flags (upper-left: low CPU usage, high GPU usage). Fix: `scontrol update partition=main TRESBillingWeights=CPU=1.0,GRES/gpu=<weight>` then `scontrol reconfigure`. Start `<weight>` near the CPUs-per-GPU ratio (128/4=32) and tune against this week's numbers (19.9% allocated, 41.5% utilized when allocated) - a policy call, so loop in whoever owns cluster allocation.
2. **Escalate on sustained low utilization** (see table and scatter above). Current top candidate (most idle GPU-hours over the last 7d): `user-0db9ced0` in `zhuang-lab` - 961 idle of 1195.5 GPU-hours allocated (20% utilization). Needs a utilization threshold (e.g. <20% mean over 50+ GPU-hours) and a grace period, then either a soft nudge (Slack/email) or a hard QOS penalty (`sacctmgr modify qos ... set Priority-=<n>`). Neither exists yet - this is read-only signal, not enforced policy.
