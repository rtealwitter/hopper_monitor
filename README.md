...
Last updated: 2026-08-26T14:00:06-07:00
Samples: 983 queue snapshots total (313 in the last 7 days), 921 GPU snapshots total (276 in the last 7 days)
...
- **21.8%** (last 7 days) vs **38.2%** (all time) of the cluster's 60 GPUs allocated, averaged across samples
...
<tr style='background-color:#d8efef'><td>witter-lab</td><td>user-554c620c</td><td align='right'>1846.0</td><td align='right'>6893.0</td><td align='right'>47%</td><td align='right'>63%</td></tr>
...
Charts below cover the trailing 7 days: **2026-08-19 14:00 to 2026-08-26 14:00** (PDT).
...
Attribution combines `nvidia-smi`'s process listing with Slurm's GPU-to-job binding record; the latter caught **1595** readings the former missed.
...
One point per user per snapshot (n=684), usage decayed on Slurm's ~7-day fairshare half-life.
...
2. **Escalate on sustained low utilization** (see table and scatter above). Current top candidate (most idle GPU-hours over the last 7d): `user-554c620c` in `witter-lab` - 971 idle of 1846.0 GPU-hours allocated (47% utilization). Needs a utilization threshold (e.g. <20% mean over 50+ GPU-hours) and a grace period, then either a soft nudge (Slack/email) or a hard QOS penalty (`sacctmgr modify qos ... set Priority-=<n>`). Neither exists yet - this is read-only signal, not enforced policy.
