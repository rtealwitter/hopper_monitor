...
Last updated: 2026-09-01T11:00:12-07:00
Samples: 1265 queue snapshots total (336 in the last 7 days), 1191 GPU snapshots total (316 in the last 7 days)
...
- **46.5%** (last 7 days) vs **41.0%** (all time) of the cluster's 60 GPUs allocated, averaged across samples
- **66.6%** (last 7 days) vs **54.1%** (all time) average `nvidia-smi` utilization *when* a GPU is allocated to a job
- **47.2%** (last 7 days) vs **69.7%** (all time) average cgroup CPU utilization *when* a CPU is allocated to a job
...
<tr style='background-color:#d8efef'><td>witter-lab</td><td>user-554c620c</td><td align='right'>4688.5</td><td align='right'>11182.0</td><td align='right'>67%</td><td align='right'>64%</td></tr>
...
Charts below cover the trailing 7 days: **2026-08-25 11:00 to 2026-09-01 11:00** (PDT).
...
Attribution combines `nvidia-smi`'s process listing with Slurm's GPU-to-job binding record; the latter caught **751** readings the former missed.
...
One point per user per snapshot (n=885), usage decayed on Slurm's ~7-day fairshare half-life.
