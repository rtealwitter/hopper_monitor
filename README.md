...
Last updated: 2026-09-10T12:30:28-07:00
Samples: 1700 queue snapshots total (336 in the last 7 days), 1534 GPU snapshots total (279 in the last 7 days)
...
- **31.7%** (last 7 days) vs **37.4%** (all time) of the cluster's 60 GPUs allocated, averaged across samples
- **51.0%** (last 7 days) vs **53.5%** (all time) average `nvidia-smi` utilization *when* a GPU is allocated to a job
- **96.2%** (last 7 days) vs **83.4%** (all time) average cgroup CPU utilization *when* a CPU is allocated to a job
...
<tr style='background-color:#ededec'><td>zhuang-lab</td><td>user-0db9ced0</td><td align='right'>1535.5</td><td align='right'>4740.5</td><td align='right'>21%</td><td align='right'>28%</td></tr>
<tr style='background-color:#d8efef'><td>witter-lab</td><td>user-554c620c</td><td align='right'>1520.5</td><td align='right'>13001.5</td><td align='right'>82%</td><td align='right'>66%</td></tr>
...
Charts below cover the trailing 7 days: **2026-09-03 12:30 to 2026-09-10 12:30** (PDT).
...
One point per user per snapshot (n=1240), usage decayed on Slurm's ~7-day fairshare half-life.
...
1. **Weight GPU usage in fairshare.** `TRESBillingWeights`/`PriorityWeightTRES` are unset, so idle GPUs cost nothing in priority - the failure mode the scatter above flags (upper-left: low CPU usage, high GPU usage). Fix: `scontrol update partition=main TRESBillingWeights=CPU=1.0,GRES/gpu=<weight>` then `scontrol reconfigure`. Start `<weight>` near the CPUs-per-GPU ratio (128/4=32) and tune against this week's numbers (31.7% allocated, 51.0% utilized when allocated) - a policy call, so loop in whoever owns cluster allocation.
2. **Escalate on sustained low utilization** (see table and scatter above). Current top candidate (most idle GPU-hours over the last 7d): `user-0db9ced0` in `zhuang-lab` - 1209 idle of 1535.5 GPU-hours allocated (21% utilization). Needs a utilization threshold (e.g. <20% mean over 50+ GPU-hours) and a grace period, then either a soft nudge (Slack/email) or a hard QOS penalty (`sacctmgr modify qos ... set Priority-=<n>`). Neither exists yet - this is read-only signal, not enforced policy.
