...
Last updated: 2026-08-19T15:30:08-07:00
Samples: 673 queue snapshots total (334 in the last 7 days), 647 GPU snapshots total (308 in the last 7 days)
...
- **38.7%** (last 7 days) vs **45.6%** (all time) of the cluster's 60 GPUs allocated, averaged across samples
- **47.0%** (last 7 days) vs **50.8%** (all time) average `nvidia-smi` utilization *when* a GPU is allocated to a job
- **81.3%** (last 7 days) vs **80.7%** (all time) average cgroup CPU utilization *when* a CPU is allocated to a job
...
<tr style='background-color:#d8efef'><td>witter-lab</td><td>user-554c620c</td><td align='right'>2375.5</td><td align='right'>5047.5</td><td align='right'>65%</td><td align='right'>69%</td></tr>
<tr style='background-color:#e3e1f1'><td>zhuang-lab</td><td>user-0db9ced0</td><td align='right'>950.0</td><td align='right'>3042.5</td><td align='right'>25%</td><td align='right'>32%</td></tr>
...
<tr style='background-color:#fbebf1'><td>nerenberg-lab</td><td>user-fedb5feb</td><td align='right'>21.0</td><td align='right'>46.0</td><td align='right'>68%</td><td align='right'>66%</td></tr>
...
Charts below cover the trailing 7 days: **2026-08-12 15:30 to 2026-08-19 15:30** (PDT).
...
Attribution combines `nvidia-smi`'s process listing with Slurm's GPU-to-job binding record; the latter caught **5418** readings the former missed.
...
One point per user per snapshot (n=1232), usage decayed on Slurm's ~7-day fairshare half-life.
...
1. **Weight GPU usage in fairshare.** `TRESBillingWeights`/`PriorityWeightTRES` are unset, so idle GPUs cost nothing in priority - the failure mode the scatter above flags (upper-left: low CPU usage, high GPU usage). Fix: `scontrol update partition=main TRESBillingWeights=CPU=1.0,GRES/gpu=<weight>` then `scontrol reconfigure`. Start `<weight>` near the CPUs-per-GPU ratio (128/4=32) and tune against this week's numbers (38.7% allocated, 47.0% utilized when allocated) - a policy call, so loop in whoever owns cluster allocation.
2. **Escalate on sustained low utilization** (see table and scatter above). Current top candidate (most idle GPU-hours over the last 7d): `user-554c620c` in `witter-lab` - 843 idle of 2375.5 GPU-hours allocated (65% utilization). Needs a utilization threshold (e.g. <20% mean over 50+ GPU-hours) and a grace period, then either a soft nudge (Slack/email) or a hard QOS penalty (`sacctmgr modify qos ... set Priority-=<n>`). Neither exists yet - this is read-only signal, not enforced policy.
