...
Last updated: 2026-08-23T17:00:04-07:00
Samples: 868 queue snapshots total (332 in the last 7 days), 832 GPU snapshots total (300 in the last 7 days)
...
- **25.4%** (last 7 days) vs **40.5%** (all time) of the cluster's 60 GPUs allocated, averaged across samples
- **38.9%** (last 7 days) vs **49.1%** (all time) average `nvidia-smi` utilization *when* a GPU is allocated to a job
...
<tr style='background-color:#d8efef'><td>witter-lab</td><td>user-554c620c</td><td align='right'>1844.0</td><td align='right'>6181.0</td><td align='right'>49%</td><td align='right'>63%</td></tr>
...
Charts below cover the trailing 7 days: **2026-08-16 17:00 to 2026-08-23 17:00** (PDT).
...
Attribution combines `nvidia-smi`'s process listing with Slurm's GPU-to-job binding record; the latter caught **2881** readings the former missed.
...
One point per user per snapshot (n=937), usage decayed on Slurm's ~7-day fairshare half-life.
...
2. **Escalate on sustained low utilization** (see table and scatter above). Current top candidate (most idle GPU-hours over the last 7d): `user-554c620c` in `witter-lab` - 932 idle of 1844.0 GPU-hours allocated (49% utilization). Needs a utilization threshold (e.g. <20% mean over 50+ GPU-hours) and a grace period, then either a soft nudge (Slack/email) or a hard QOS penalty (`sacctmgr modify qos ... set Priority-=<n>`). Neither exists yet - this is read-only signal, not enforced policy.
