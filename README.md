...
Last updated: 2026-08-30T09:30:29-07:00
Samples: 1166 queue snapshots total (313 in the last 7 days), 1092 GPU snapshots total (275 in the last 7 days)
...
- **34.1%** (last 7 days) vs **38.9%** (all time) of the cluster's 60 GPUs allocated, averaged across samples
- **60.7%** (last 7 days) vs **51.7%** (all time) average `nvidia-smi` utilization *when* a GPU is allocated to a job
- **51.2%** (last 7 days) vs **74.2%** (all time) average cgroup CPU utilization *when* a CPU is allocated to a job
...
<tr style='background-color:#d8efef'><td>witter-lab</td><td>user-554c620c</td><td align='right'>3200.5</td><td align='right'>9232.5</td><td align='right'>61%</td><td align='right'>63%</td></tr>
...
Charts below cover the trailing 7 days: **2026-08-23 09:30 to 2026-08-30 09:30** (PDT).
...
Attribution combines `nvidia-smi`'s process listing with Slurm's GPU-to-job binding record; the latter caught **981** readings the former missed.
...
One point per user per snapshot (n=661), usage decayed on Slurm's ~7-day fairshare half-life.
