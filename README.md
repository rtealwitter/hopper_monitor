...
Last updated: 2026-08-26T00:30:11-07:00
Samples: 956 queue snapshots total (313 in the last 7 days), 895 GPU snapshots total (269 in the last 7 days)
...
- **65.1%** (last 7 days) vs **77.7%** (all time) average cgroup CPU utilization *when* a CPU is allocated to a job
...
<tr style='background-color:#d8efef'><td>witter-lab</td><td>user-554c620c</td><td align='right'>1699.0</td><td align='right'>6678.5</td><td align='right'>45%</td><td align='right'>63%</td></tr>
...
Charts below cover the trailing 7 days: **2026-08-19 00:30 to 2026-08-26 00:30** (PDT).
...
Attribution combines `nvidia-smi`'s process listing with Slurm's GPU-to-job binding record; the latter caught **1695** readings the former missed.
...
One point per user per snapshot (n=709), usage decayed on Slurm's ~7-day fairshare half-life.
...
2. **Escalate on sustained low utilization** (see table and scatter above). Current top candidate (most idle GPU-hours over the last 7d): `user-554c620c` in `witter-lab` - 931 idle of 1699.0 GPU-hours allocated (45% utilization). Needs a utilization threshold (e.g. <20% mean over 50+ GPU-hours) and a grace period, then either a soft nudge (Slack/email) or a hard QOS penalty (`sacctmgr modify qos ... set Priority-=<n>`). Neither exists yet - this is read-only signal, not enforced policy.
