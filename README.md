...
Last updated: 2026-09-11T19:00:22-07:00
Samples: 1761 queue snapshots total (336 in the last 7 days), 1595 GPU snapshots total (333 in the last 7 days)
...
- **47.6%** (last 7 days) vs **39.1%** (all time) of the cluster's 60 GPUs allocated, averaged across samples
- **57.6%** (last 7 days) vs **54.8%** (all time) average `nvidia-smi` utilization *when* a GPU is allocated to a job
- **96.9%** (last 7 days) vs **84.5%** (all time) average cgroup CPU utilization *when* a CPU is allocated to a job
...
<tr style='background-color:#d8efef'><td>witter-lab</td><td>user-554c620c</td><td align='right'>2780.5</td><td align='right'>14261.5</td><td align='right'>83%</td><td align='right'>68%</td></tr>
<tr style='background-color:#ededec'><td>zhuang-lab</td><td>user-0db9ced0</td><td align='right'>1877.5</td><td align='right'>5082.5</td><td align='right'>22%</td><td align='right'>28%</td></tr>
...
Charts below cover the trailing 7 days: **2026-09-04 19:00 to 2026-09-11 19:00** (PDT).
...
One point per user per snapshot (n=1379), usage decayed on Slurm's ~7-day fairshare half-life.
...
1. **Weight GPU usage in fairshare.** `TRESBillingWeights`/`PriorityWeightTRES` are unset, so idle GPUs cost nothing in priority - the failure mode the scatter above flags (upper-left: low CPU usage, high GPU usage). Fix: `scontrol update partition=main TRESBillingWeights=CPU=1.0,GRES/gpu=<weight>` then `scontrol reconfigure`. Start `<weight>` near the CPUs-per-GPU ratio (128/4=32) and tune against this week's numbers (47.6% allocated, 57.6% utilized when allocated) - a policy call, so loop in whoever owns cluster allocation.
2. **Escalate on sustained low utilization** (see table and scatter above). Current top candidate (most idle GPU-hours over the last 7d): `user-0db9ced0` in `zhuang-lab` - 1455 idle of 1877.5 GPU-hours allocated (22% utilization). Needs a utilization threshold (e.g. <20% mean over 50+ GPU-hours) and a grace period, then either a soft nudge (Slack/email) or a hard QOS penalty (`sacctmgr modify qos ... set Priority-=<n>`). Neither exists yet - this is read-only signal, not enforced policy.
