...
Last updated: 2026-08-23T02:30:14-07:00
Samples: 839 queue snapshots total (332 in the last 7 days), 803 GPU snapshots total (300 in the last 7 days)
...
- **42.1%** (last 7 days) vs **49.6%** (all time) average `nvidia-smi` utilization *when* a GPU is allocated to a job
- **72.6%** (last 7 days) vs **78.9%** (all time) average cgroup CPU utilization *when* a CPU is allocated to a job
...
<tr style='background-color:#d8efef'><td>witter-lab</td><td>user-554c620c</td><td align='right'>1849.0</td><td align='right'>5943.0</td><td align='right'>54%</td><td align='right'>65%</td></tr>
...
Charts below cover the trailing 7 days: **2026-08-16 02:30 to 2026-08-23 02:30** (PDT).
...
Attribution combines `nvidia-smi`'s process listing with Slurm's GPU-to-job binding record; the latter caught **3073** readings the former missed.
...
2. **Escalate on sustained low utilization** (see table and scatter above). Current top candidate (most idle GPU-hours over the last 7d): `user-554c620c` in `witter-lab` - 851 idle of 1849.0 GPU-hours allocated (54% utilization). Needs a utilization threshold (e.g. <20% mean over 50+ GPU-hours) and a grace period, then either a soft nudge (Slack/email) or a hard QOS penalty (`sacctmgr modify qos ... set Priority-=<n>`). Neither exists yet - this is read-only signal, not enforced policy.
