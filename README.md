...
Last updated: 2026-09-04T15:30:07-07:00
Samples: 1418 queue snapshots total (336 in the last 7 days), 1258 GPU snapshots total (250 in the last 7 days)
...
- **72.7%** (last 7 days) vs **74.2%** (all time) average cgroup CPU utilization *when* a CPU is allocated to a job
...
<tr style='background-color:#d8efef'><td>witter-lab</td><td>user-554c620c</td><td align='right'>2771.0</td><td align='right'>11481.0</td><td align='right'>66%</td><td align='right'>64%</td></tr>
...
Charts below cover the trailing 7 days: **2026-08-28 15:30 to 2026-09-04 15:30** (PDT).
...
One point per user per snapshot (n=957), usage decayed on Slurm's ~7-day fairshare half-life.
...
2. **Escalate on sustained low utilization** (see table and scatter above). Current top candidate (most idle GPU-hours over the last 7d): `user-554c620c` in `witter-lab` - 940 idle of 2771.0 GPU-hours allocated (66% utilization). Needs a utilization threshold (e.g. <20% mean over 50+ GPU-hours) and a grace period, then either a soft nudge (Slack/email) or a hard QOS penalty (`sacctmgr modify qos ... set Priority-=<n>`). Neither exists yet - this is read-only signal, not enforced policy.
