...
Last updated: 2026-08-29T16:30:14-07:00
Samples: 1132 queue snapshots total (313 in the last 7 days), 1058 GPU snapshots total (275 in the last 7 days)
...
Based on 25 days of history so far, `hopper.cluster` has historically been most open on **Saturdays** (23.6% of GPUs allocated on average) and around **08:00-09:00** (32.4% of GPUs allocated on average). Recomputed from all-time data on every cron tick, so it sharpens as more history accumulates.
...
<tr style='background-color:#d8efef'><td>witter-lab</td><td>user-554c620c</td><td align='right'>3075.5</td><td align='right'>8832.0</td><td align='right'>57%</td><td align='right'>63%</td></tr>
<tr style='background-color:#fae3e3'><td>zhuang-lab</td><td>user-0db9ced0</td><td align='right'>5.0</td><td align='right'>3205.0</td><td align='right'>0%</td><td align='right'>31%</td></tr>
...
Charts below cover the trailing 7 days: **2026-08-22 16:30 to 2026-08-29 16:30** (PDT).
...
Attribution combines `nvidia-smi`'s process listing with Slurm's GPU-to-job binding record; the latter caught **1103** readings the former missed.
...
One point per user per snapshot (n=595), usage decayed on Slurm's ~7-day fairshare half-life.
...
2. **Escalate on sustained low utilization** (see table and scatter above). Current top candidate (most idle GPU-hours over the last 7d): `user-554c620c` in `witter-lab` - 1317 idle of 3075.5 GPU-hours allocated (57% utilization). Needs a utilization threshold (e.g. <20% mean over 50+ GPU-hours) and a grace period, then either a soft nudge (Slack/email) or a hard QOS penalty (`sacctmgr modify qos ... set Priority-=<n>`). Neither exists yet - this is read-only signal, not enforced policy.
