...
Last updated: 2026-09-12T02:00:10-07:00
Samples: 1775 queue snapshots total (336 in the last 7 days), 1609 GPU snapshots total (335 in the last 7 days)
...
Based on 39 days of history so far, `hopper.cluster` has historically been most open on **Saturdays** (24.1% of GPUs allocated on average) and around **08:00-09:00** (32.8% of GPUs allocated on average). Recomputed from all-time data on every cron tick, so it sharpens as more history accumulates.
...
<tr style='background-color:#d8efef'><td>witter-lab</td><td>user-554c620c</td><td align='right'>2832.5</td><td align='right'>14318.5</td><td align='right'>83%</td><td align='right'>68%</td></tr>
<tr style='background-color:#ededec'><td>zhuang-lab</td><td>user-0db9ced0</td><td align='right'>1920.5</td><td align='right'>5180.0</td><td align='right'>23%</td><td align='right'>28%</td></tr>
...
Charts below cover the trailing 7 days: **2026-09-05 02:00 to 2026-09-12 02:00** (PDT).
...
Attribution combines `nvidia-smi`'s process listing with Slurm's GPU-to-job binding record; the latter caught **857** readings the former missed.
...
2. **Escalate on sustained low utilization** (see table and scatter above). Current top candidate (most idle GPU-hours over the last 7d): `user-0db9ced0` in `zhuang-lab` - 1480 idle of 1920.5 GPU-hours allocated (23% utilization). Needs a utilization threshold (e.g. <20% mean over 50+ GPU-hours) and a grace period, then either a soft nudge (Slack/email) or a hard QOS penalty (`sacctmgr modify qos ... set Priority-=<n>`). Neither exists yet - this is read-only signal, not enforced policy.
