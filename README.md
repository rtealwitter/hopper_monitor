...
Last updated: 2026-09-18T03:30:13-07:00
Samples: 2066 queue snapshots total (336 in the last 7 days), 1857 GPU snapshots total (293 in the last 7 days)
...
- **33.3%** (last 7 days) vs **37.5%** (all time) of the cluster's 60 GPUs allocated, averaged across samples
- **42.0%** (last 7 days) vs **52.7%** (all time) average `nvidia-smi` utilization *when* a GPU is allocated to a job
- **95.3%** (last 7 days) vs **87.2%** (all time) average cgroup CPU utilization *when* a CPU is allocated to a job

## Most open times

Based on 45 days of history so far, `hopper.cluster` has historically been most open on **Saturdays** (27.0% of GPUs allocated on average) and around **08:00-09:00** (32.5% of GPUs allocated on average). Recomputed from all-time data on every cron tick, so it sharpens as more history accumulates.
...
<tr style='background-color:#ededec'><td>zhuang-lab</td><td>user-0db9ced0</td><td align='right'>1663.0</td><td align='right'>6552.5</td><td align='right'>22%</td><td align='right'>26%</td></tr>
<tr style='background-color:#d8efef'><td>witter-lab</td><td>user-554c620c</td><td align='right'>1132.0</td><td align='right'>14804.0</td><td align='right'>70%</td><td align='right'>67%</td></tr>
<tr style='background-color:#e3e1f1'><td>nerenberg-lab</td><td>user-7eb22d7c</td><td align='right'>499.0</td><td align='right'>568.5</td><td align='right'>41%</td><td align='right'>67%</td></tr>
<tr style='background-color:#e3e1f1'><td>nerenberg-lab</td><td>user-fedb5feb</td><td align='right'>46.5</td><td align='right'>158.5</td><td align='right'>66%</td><td align='right'>59%</td></tr>
<tr style='background-color:#ededec'><td>zhuang-lab</td><td>user-7d156b54</td><td align='right'>10.0</td><td align='right'>32.0</td><td align='right'>38%</td><td align='right'>39%</td></tr>
...
<tr style='background-color:#e3e1f1'><td>nerenberg-lab</td><td>user-b12dc074</td><td align='right'>2.0</td><td align='right'>2.5</td><td align='right'>45%</td><td align='right'>51%</td></tr>
...
Charts below cover the trailing 7 days: **2026-09-11 03:30 to 2026-09-18 03:30** (PDT).
...
Attribution combines `nvidia-smi`'s process listing with Slurm's GPU-to-job binding record; the latter caught **748** readings the former missed.
...
One point per user per snapshot (n=1523), usage decayed on Slurm's ~7-day fairshare half-life.
...
1. **Weight GPU usage in fairshare.** `TRESBillingWeights`/`PriorityWeightTRES` are unset, so idle GPUs cost nothing in priority - the failure mode the scatter above flags (upper-left: low CPU usage, high GPU usage). Fix: `scontrol update partition=main TRESBillingWeights=CPU=1.0,GRES/gpu=<weight>` then `scontrol reconfigure`. Start `<weight>` near the CPUs-per-GPU ratio (128/4=32) and tune against this week's numbers (33.3% allocated, 42.0% utilized when allocated) - a policy call, so loop in whoever owns cluster allocation.
2. **Escalate on sustained low utilization** (see table and scatter above). Current top candidate (most idle GPU-hours over the last 7d): `user-0db9ced0` in `zhuang-lab` - 1293 idle of 1663.0 GPU-hours allocated (22% utilization). Needs a utilization threshold (e.g. <20% mean over 50+ GPU-hours) and a grace period, then either a soft nudge (Slack/email) or a hard QOS penalty (`sacctmgr modify qos ... set Priority-=<n>`). Neither exists yet - this is read-only signal, not enforced policy.
