...
Last updated: 2026-08-06T12:00:19-07:00
Samples: 44 queue snapshots, 44 GPU snapshots
...
- **70.3%** of the cluster's 60 GPUs allocated, averaged across all samples
- **39.8%** average `nvidia-smi` utilization *when* a GPU is allocated to a job
- **82.0%** average cgroup CPU utilization *when* a CPU is allocated to a job
...
<tr style='background-color:#fbebf1'><td>zhuang-lab</td><td>user-0db9ced0</td><td align='right'>336.0</td><td align='right'>336.0</td><td align='right'>48%</td></tr>
<tr style='background-color:#d8efef'><td>witter-lab</td><td>user-554c620c</td><td align='right'>85.0</td><td align='right'>196.0</td><td align='right'>70%</td></tr>
...
<tr style='background-color:#fce8e0'><td>ibarragarciapadilla-lab</td><td>user-3cfc41a3</td><td align='right'>0.0</td><td align='right'>2437.5</td><td align='right'>—</td></tr>
...
Attribution is cross-referenced two ways: `nvidia-smi`'s own process listing (misses containerized/namespaced processes), backfilled from Slurm's GPU-to-job binding record (`scontrol show job -dd`). The scontrol fallback attributed **1413** GPU readings this run that the process-listing path missed.
...
Each point is one user at one snapshot (n=255), not averaged over time, to show how a user's position moves rather than collapsing it to a single number. Both axes are usage decayed with Slurm's own ~7-day fairshare half-life (`PriorityDecayHalfLife` on this cluster), not a lifetime total or a per-day average.
