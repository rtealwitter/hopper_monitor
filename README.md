...
Last updated: 2026-08-26T09:00:10-07:00
Samples: 973 queue snapshots total (313 in the last 7 days), 912 GPU snapshots total (270 in the last 7 days)
...
- **44.4%** (last 7 days) vs **49.7%** (all time) average `nvidia-smi` utilization *when* a GPU is allocated to a job
- **62.9%** (last 7 days) vs **77.2%** (all time) average cgroup CPU utilization *when* a CPU is allocated to a job
...
<tr style='background-color:#d8efef'><td>witter-lab</td><td>user-554c620c</td><td align='right'>1819.5</td><td align='right'>6848.0</td><td align='right'>47%</td><td align='right'>63%</td></tr>
...
Charts below cover the trailing 7 days: **2026-08-19 09:00 to 2026-08-26 09:00** (PDT).
...
One point per user per snapshot (n=692), usage decayed on Slurm's ~7-day fairshare half-life.
...
2. **Escalate on sustained low utilization** (see table and scatter above). Current top candidate (most idle GPU-hours over the last 7d): `user-554c620c` in `witter-lab` - 966 idle of 1819.5 GPU-hours allocated (47% utilization). Needs a utilization threshold (e.g. <20% mean over 50+ GPU-hours) and a grace period, then either a soft nudge (Slack/email) or a hard QOS penalty (`sacctmgr modify qos ... set Priority-=<n>`). Neither exists yet - this is read-only signal, not enforced policy.
