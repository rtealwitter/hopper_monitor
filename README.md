...
Last updated: 2026-09-16T20:30:19-07:00
Samples: 2004 queue snapshots total (336 in the last 7 days), 1796 GPU snapshots total (294 in the last 7 days)
...
- **56.9%** (last 7 days) vs **53.3%** (all time) average `nvidia-smi` utilization *when* a GPU is allocated to a job
- **95.0%** (last 7 days) vs **86.3%** (all time) average cgroup CPU utilization *when* a CPU is allocated to a job
...
<tr style='background-color:#d8efef'><td>witter-lab</td><td>user-554c620c</td><td align='right'>2374.5</td><td align='right'>14729.0</td><td align='right'>81%</td><td align='right'>68%</td></tr>
<tr style='background-color:#ededec'><td>zhuang-lab</td><td>user-0db9ced0</td><td align='right'>1651.0</td><td align='right'>6238.0</td><td align='right'>24%</td><td align='right'>27%</td></tr>
<tr style='background-color:#e3e1f1'><td>nerenberg-lab</td><td>user-7eb22d7c</td><td align='right'>399.0</td><td align='right'>456.5</td><td align='right'>41%</td><td align='right'>67%</td></tr>
...
Charts below cover the trailing 7 days: **2026-09-09 20:30 to 2026-09-16 20:30** (PDT).
...
1. **Weight GPU usage in fairshare.** `TRESBillingWeights`/`PriorityWeightTRES` are unset, so idle GPUs cost nothing in priority - the failure mode the scatter above flags (upper-left: low CPU usage, high GPU usage). Fix: `scontrol update partition=main TRESBillingWeights=CPU=1.0,GRES/gpu=<weight>` then `scontrol reconfigure`. Start `<weight>` near the CPUs-per-GPU ratio (128/4=32) and tune against this week's numbers (44.5% allocated, 56.9% utilized when allocated) - a policy call, so loop in whoever owns cluster allocation.
2. **Escalate on sustained low utilization** (see table and scatter above). Current top candidate (most idle GPU-hours over the last 7d): `user-0db9ced0` in `zhuang-lab` - 1251 idle of 1651.0 GPU-hours allocated (24% utilization). Needs a utilization threshold (e.g. <20% mean over 50+ GPU-hours) and a grace period, then either a soft nudge (Slack/email) or a hard QOS penalty (`sacctmgr modify qos ... set Priority-=<n>`). Neither exists yet - this is read-only signal, not enforced policy.
